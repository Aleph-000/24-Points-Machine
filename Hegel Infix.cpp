...
// Forward declaration (used in Solver)
static void print_infix(const vector<string>& expr, const string& prefix = "");
// Forward declaration (used in simplification)
static bool is_perfect_square_ll(long long x, long long& r);

// ======================= 可调参数 =======================
// 最大函数嵌套深度（sqrt(lg(100)) 这种算2层）
static const int MAX_NEST = 4;

// 每种函数最多出现次数（建议 1~2）
static const int MAX_USE_SQRT = 2;
static const int MAX_USE_FACT = 2; // 为了支持 24!/23! 默认给2
static const int MAX_USE_LG   = 1;
static const int MAX_USE_LB   = 2;
static const int MAX_USE_LOG  = 1; // log_a(b)

// 大数剪枝：普通整数值只保留到这个范围（加减强制用它剪枝）
static const long long MAX_ABS_VAL = 10000;

// 阶乘自变量上限（n! 仅当 0<=n<=MAX_FACT_ARG 允许）
static const int MAX_FACT_ARG = 50;

// 乘除形成的质因子指数总和过大就剪掉（防止爆炸）
static const int MAX_EXP_SUM = 100;

// 正常模式是否只找一个解（false=尽量找全；true=找到一个就停）
static const bool NORMAL_FIND_FIRST_ONLY = false;

// 是否在“找全解”时也启用记忆化（true更快，但可能漏掉等价状态下的不同RPN解）
static const bool MEMO_IN_FIND_ALL = true;

// 等价化简步数：在给定步数内可化简到同一常数的表达式视为同一种
static const int SIMPLIFY_STEPS = 2;

// 激进剪枝参数：搜索时跳过等价表达式
static const bool SKIP_EQUIV_DURING_SEARCH = true;

// 禁止中间负数
static const bool NO_NEGATIVE_INTERMEDIATE = true;

// 仅使用四则运算
static const bool ONLY_ARITHMETIC = false;

// 等价 key 的缓存上限，防止缓存无限增长占内存
static const size_t MAX_EQUIV_KEY_CACHE = 200000;

// 目标值
static const long long TARGET = 24;
// ======================================================================

enum FuncIdx { F_SQRT = 0, F_FACT = 1, F_LG = 2, F_LB = 3, F_LOG = 4, F_CNT = 5 };

static const array<int, F_CNT> MAX_USE = {
    MAX_USE_SQRT, MAX_USE_FACT, MAX_USE_LG, MAX_USE_LB, MAX_USE_LOG
};

struct Num {
    int sign = 1; // -1, 0, +1  (0表示数值为0)
    vector<pair<int, int>> pe; // abs(value) 的质因数分解：prime->exp (exp>0)
    bool has_ll = false;
    long long ll = 0; // 若 has_ll=true，则保证 |ll|<=MAX_ABS_VAL
};
...
// --------------- 工具：RPN 拼接 ---------------
static inline vector<string> merge_expr(const vector<string>& a, const vector<string>& b, const string& op) {
    vector<string> res;
...
static inline vector<string> apply_unary_expr(const vector<string>& a, const string& op) {
    vector<string> res = a;
    res.push_back(op);
    return res;
}
...
static int build_expr_tree(const vector<string>& expr, vector<ExprNodeTmp>& nodes) {
    vector<int> st;
    ...
    for (const string& t : expr) {
        if (is_binary_token(t)) {
            ...
            ExprNodeTmp n; n.op = t; n.left = a; n.right = b; n.is_binary = true;
            nodes.push_back(std::move(n));
            st.push_back((int)nodes.size() - 1);
        } else if (is_unary_token(t)) {
            if (st.empty()) return -1;
            int a = st.back(); st.pop_back();
            ExprNodeTmp n; n.op = t; n.child = a; n.is_unary = true;
            nodes.push_back(std::move(n));
            st.push_back((int)nodes.size() - 1);
        } else {
            ExprNodeTmp n; n.tok = t; n.is_leaf = true;
            nodes.push_back(std::move(n));
            st.push_back((int)nodes.size() - 1);
        }
    }
...
static bool safe_add_ll(long long a, long long b, long long& out) {
    ...
    if (r < LLONG_MIN || r > LLONG_MAX) return false;
    out = (long long)r;
    return true;
}
...
static bool safe_fact_ll(long long n, long long& out) {
    if (n < 0 || n > MAX_FACT_ARG) return false;
    __int128 r = 1;
    for (long long i = 2; i <= n; i++) {
        r *= i;
        if (r > LLONG_MAX) return false;
    }
    out = (long long)r;
    return true;
}
...
static bool safe_log_ll(long long a, long long b, long long& out) {
    ...
    if (b == 1) { out = 0; return true; }
    long long k = 0;
    __int128 cur = 1;
    while (cur < b) {
        cur *= a;
        k++;
        if (cur > LLONG_MAX) return false;
    }
...
static bool simplify_dfs(int idx, long long& out_val, int& out_steps) {
    if (SIMPLIFY_STEPS <= 0 || g_simpl.nodes == nullptr) return false;
    if (g_simpl.vis[idx]) {
        if (g_simpl.ok[idx]) {
    ...
    if (n.is_leaf) {
        ...
        if (simplify_dfs(n.child, cv, cs)) {
            if (cs + 1 <= SIMPLIFY_STEPS) {
                if (n.op == "sqrt") {
                    long long r;
                    if (is_perfect_square_ll(cv, r)) { v = r; ok = true; steps = cs + 1; }
                } else if (n.op == "!") {
                    if (safe_fact_ll(cv, v)) { ok = true; steps = cs + 1; }
                } else if (n.op == "lg") {
                    if (safe_lg_ll(cv, v)) { ok = true; steps = cs + 1; }
                } else if (n.op == "lb") {
                    if (safe_lb_ll(cv, v)) { ok = true; steps = cs + 1; }
                }
            }
        }
    } else if (n.is_binary) {
        long long lv, rv; int ls, rs;
        if (simplify_dfs(n.left, lv, ls) && simplify_dfs(n.right, rv, rs)) {
            if (ls + rs + 1 <= SIMPLIFY_STEPS) {
                if (n.op == "+") {
                    if (safe_add_ll(lv, rv, v)) { ok = true; steps = ls + rs + 1; }
                } else if (n.op == "-") {
                    if (safe_sub_ll(lv, rv, v)) { ok = true; steps = ls + rs + 1; }
                } else if (n.op == "*") {
                    if (safe_mul_ll(lv, rv, v)) { ok = true; steps = ls + rs + 1; }
                } else if (n.op == "/") {
                    if (rv != 0 && (lv % rv == 0)) { v = lv / rv; ok = true; steps = ls + rs + 1; }
                } else if (n.op == "log") {
                    if (safe_log_ll(lv, rv, v)) { ok = true; steps = ls + rs + 1; }
                }
            }
        }
    }
...
static bool try_simplify_const(const vector<ExprNodeTmp>& nodes, int idx, long long& out_val) {
    if (!SKIP_EQUIV_DURING_SEARCH) return false;
    if (SIMPLIFY_STEPS <= 0) return false;
    if (g_simpl.nodes != &nodes) init_simplify_cache(nodes);
    int steps = 0;
    return simplify_dfs(idx, out_val, steps);
}

static string normalized_expr_key_from_nodes(const vector<ExprNodeTmp>& nodes, int idx);

static void collect_mul_factors(const vector<ExprNodeTmp>& nodes, int idx, int sign, vector<string>& factors) {
...
static string normalized_mul_key(const vector<ExprNodeTmp>& nodes, int idx, bool& direct_key) {
...
static void collect_add_terms(const vector<ExprNodeTmp>& nodes, int idx, int sign, vector<string>& terms) {
    const ExprNodeTmp& n = nodes[idx];
    if (n.is_binary && (n.op == "+" || n.op == "-")) {
...
static string normalized_add_key_from_nodes(const vector<ExprNodeTmp>& nodes, int idx, bool& direct_key) {
    vector<string> terms;
    terms.reserve(8);
    ...
    sort(b.begin(), b.end());
    string sa, sb;
    for (size_t i = 0; i < a.size(); i++) { if (i) sa.push_back('|'); sa += a[i]; }
    for (size_t i = 0; i < b.size(); i++) { if (i) sb.push_back('|'); sb += b[i]; }
    bool neg = false;
    vector<string>* use = &a;
    if (!sb.empty() && sb < sa) { neg = true; use = &b; }
...
static string normalized_expr_key_from_nodes(const vector<ExprNodeTmp>& nodes, int idx) {
    long long sv;
    if (try_simplify_const(nodes, idx, sv)) return to_string(sv);
    const ExprNodeTmp& n = nodes[idx];
    if (n.is_leaf) return n.tok;
    if (n.is_unary) return n.op + "(" + normalized_expr_key_from_nodes(nodes, n.child) + ")";
    if (n.is_binary) {
        if (n.op == "+" || n.op == "-") {
            bool direct_key = false;
            string ak = normalized_add_key_from_nodes(nodes, idx, direct_key);
            if (direct_key) return ak;
            if (!ak.empty() && ak[0] == '-') return string("-") + string("A:") + ak.substr(1);
            return string("A:") + ak;
        }
        if (n.op == "*" || n.op == "/") {
            bool direct_key = false;
            string mk = normalized_mul_key(nodes, idx, direct_key);
            if (direct_key) return mk;
            return string("M:") + mk;
        }
        return n.op + "(" + normalized_expr_key_from_nodes(nodes, n.left) + "," + normalized_expr_key_from_nodes(nodes, n.right) + ")";
    }
    return "";
}
static string normalized_expr_key(const vector<string>& expr) {
    ...
    for (size_t i = 0; i < expr.size(); i++) {
        if (i) rpn_key.push_back(' ');
        rpn_key += expr[i];
    }
    auto it = equiv_key_cache.find(rpn_key);
    if (it != equiv_key_cache.end()) return it->second;
...
static int count_leaf_tokens(const vector<string>& expr) {
    int c = 0;
    for (const string& t : expr) {
        if (!is_binary_token(t) && !is_unary_token(t)) c++;
    }
    return c;
}
...
static vector<pair<int, int>> factorize_small(long long x) {
    ...
    if (x > 1) res.push_back({ (int)x, 1 });
    return res;
}

static int exp_sum(const vector<pair<int, int>>& pe) {
    int s = 0;
    for (auto& kv : pe) s += kv.second;
    return s;
}

// factors: out = a + b (指数相加)
static vector<pair<int, int>> factors_add(const vector<pair<int, int>>& a, const vector<pair<int, int>>& b) {
...
static bool factors_subtract(const vector<pair<int, int>>& a, const vector<pair<int, int>>& b, vector<pair<int, int>>& out) {
    out.clear();
    out.reserve(a.size());
    size_t i = 0, j = 0;
    while (i < a.size() || j < b.size()) {
...
static bool try_eval_small_abs(const vector<pair<int, int>>& pe, long long& out_abs) {
    __int128 prod = 1;
    for (auto& kv : pe) {
        int p = kv.first;
        int e = kv.second;
        for (int i = 0; i < e; i++) {
            prod *= p;
            if (prod > MAX_ABS_VAL) return false;
        }
    }
...
static Num make_num_from_ll_pruned(long long v, bool& ok) {
    ok = false;
    Num n;
    if (NO_NEGATIVE_INTERMEDIATE && v < 0) return n; // forbid negative intermediates
    if (v == 0) {
        n.sign = 0;
        n.pe.clear();
        n.has_ll = true;
    ...
    if (av > MAX_ABS_VAL) return n; // 剪枝：普通整数太大直接丢
    n.sign = (v < 0 ? -1 : 1);
...
static vector<int> primes_up_to(int n) {
    ...
    for (int i = 2; i <= n; i++) if (isPrime[i]) ps.push_back(i);
    return ps;
}
...
// --------------- 函数使用次数合并检查 ---------------
static bool merge_used(const Node& A, const Node& B, array<unsigned char, F_CNT>& out) {
    for (int i = 0; i < F_CNT; i++) {
        int s = (int)A.used[i] + (int)B.used[i];
        if (s > MAX_USE[i]) return false;
        out[i] = (unsigned char)s;
    }
    return true;
}
static bool inc_used(const Node& A, FuncIdx f, array<unsigned char, F_CNT>& out) {
    out = A.used;
    if ((int)out[f] + 1 > MAX_USE[f]) return false;
    out[f] = (unsigned char)(out[f] + 1);
    return true;
}

// --------------- 目标判断（不依赖大整数） ---------------
static const vector<pair<int, int>> TARGET_FACTORS = factorize_small(TARGET);

static bool is_target_24(const Num& n) {
    if (n.sign <= 0) return false;
    if (n.has_ll) return n.ll == TARGET;
...
static inline bool need_paren_right(char op, const InfixItem& b) {
    int p = prec_of_binop(op);
    if (op == '+') {
        if (b.prec < p) return true;
        // a + (b - c) 必须加括号
        if (b.prec == p && b.bop == '-') return true;
        return false;
    }
    if (op == '-') {
        // a - (b +/- c) 必须加括号
...
static string rpn_to_infix(const vector<string>& expr) {
    ...
    for (const string& t : expr) {
        if (t == "+" || t == "-" || t == "*" || t == "/") {
            ...
            InfixItem b = std::move(st.back()); st.pop_back();
            InfixItem a = std::move(st.back()); st.pop_back();
            char op = t[0];
...
struct Solver {
    ...
    static string state_key(const vector<Node>& cur) {
        vector<string> ks;
        ks.reserve(cur.size());
        for (auto& nd : cur) ks.push_back(node_key(nd));
    ...
    bool try_sqrt(const Node& A, Node& out) {
        if (A.depth + 1 > MAX_NEST) return false;
        array<unsigned char, F_CNT> used2;
        if (!inc_used(A, F_SQRT, used2)) return false;
        if (!A.num.has_ll) return false;
        long long v = A.num.ll;
        if (v == 0 || v == 1) return false;
        long long r;
        if (!is_perfect_square_ll(v, r)) return false;
    ...
    bool try_fact(const Node& A, Node& out) {
        if (A.depth + 1 > MAX_NEST) return false;
        array<unsigned char, F_CNT> used2;
        if (!inc_used(A, F_FACT, used2)) return false;
        if (!A.num.has_ll) return false;
        long long v = A.num.ll;
        if (v < 0 || v > MAX_FACT_ARG) return false;
        ...
        if (v == 0 || v == 1) {
            ...
            normalize_num(n);
            if (!n.has_ll && exp_sum(n.pe) > MAX_EXP_SUM) return false;
        }
    ...
    bool try_lg(const Node& A, Node& out) {
        if (A.depth + 1 > MAX_NEST) return false;
        array<unsigned char, F_CNT> used2;
        if (!inc_used(A, F_LG, used2)) return false;
        if (!A.num.has_ll) return false;
        long long v = A.num.ll;
        if (v <= 0 || v == 1) return false;
        if (v == 4 || v == 16) return false;
    ...
    bool try_lb(const Node& A, Node& out) {
        if (A.depth + 1 > MAX_NEST) return false;
        array<unsigned char, F_CNT> used2;
        if (!inc_used(A, F_LB, used2)) return false;
        if (!A.num.has_ll) return false;
        long long v = A.num.ll;
        if (v <= 0 || v == 1) return false;
        if (v == 4 || v == 16) return false;
    ...
    bool try_logab(const Node& A, const Node& B, Node& out) {
        ...
        if (newDepth > MAX_NEST) return false;

        array<unsigned char, F_CNT> used2;
        if (!merge_used(A, B, used2)) return false;
        if ((int)used2[F_LOG] + 1 > MAX_USE[F_LOG]) return false;
        used2[F_LOG] = (unsigned char)(used2[F_LOG] + 1);

        if (!A.num.has_ll || !B.num.has_ll) return false;
    ...
    bool try_add(const Node& A, const Node& B, Node& out) {
        ...
        if (r > MAX_ABS_VAL || r < -MAX_ABS_VAL) return false;
    ...
    bool try_mul(const Node& A, const Node& B, Node& out) {
        ...
        if (A.num.sign == 0 || B.num.sign == 0) {
            ...
            if (exp_sum(n.pe) > MAX_EXP_SUM) return false;
            normalize_num(n);
        }
    ...
    bool try_div(const Node& A, const Node& B, Node& out) {
        ...
        if (B.num.sign == 0) return false;
        if (B.num.has_ll && B.num.ll == 1) return false;
        ...
        if (A.num.sign == 0) {
            ...
            if (!factors_subtract(A.num.pe, B.num.pe, pe2)) return false; // 必须整除
            n.sign = A.num.sign * B.num.sign;
            n.pe = std::move(pe2);
            normalize_num(n);
            if (!n.has_ll && exp_sum(n.pe) > MAX_EXP_SUM) return false;
    ...
    void dfs(vector<Node> cur) {
        ...
        if (cur.size() == 1) {
            if (is_target_24(cur[0].num) && count_leaf_tokens(cur[0].expr) == expected_leaf_count) {
                if (find_first) {
                    found = true;
        ...
        for (int i = 0; i < n - 1; i++) for (int j = i + 1; j < n; j++) {
            ...
            pair_key.push_back('|');
            pair_key += keyB;
            if (!pair_seen.insert(pair_key).second) continue;

            vector<Node> rest;
            rest.reserve(n - 1);
            for (int k = 0; k < n; k++) if (k != i && k != j) rest.push_back(cur[k]);
            ...
            { Node C; if (try_add(A, B, C)) { push_and_dfs(std::move(C)); if (found && find_first) return; } }
            {
                Node C;
                if (try_sub(A, B, C)) { push_and_dfs(std::move(C)); if (found && find_first) return; }
                if (try_sub(B, A, C)) { push_and_dfs(std::move(C)); if (found && find_first) return; }
            }
            { Node C; if (try_mul(A, B, C)) { push_and_dfs(std::move(C)); if (found && find_first) return; } }
            {
                Node C;
                if (try_div(A, B, C)) { push_and_dfs(std::move(C)); if (found && find_first) return; }
                if (try_div(B, A, C)) { push_and_dfs(std::move(C)); if (found && find_first) return; }
            }
            {
                Node C;
                if (try_logab(A, B, C)) { push_and_dfs(std::move(C)); if (found && find_first) return; }
                if (try_logab(B, A, C)) { push_and_dfs(std::move(C)); if (found && find_first) return; }
            }
        }
    }
    ...
    bool solve_first(const vector<long long>& nums, vector<string>& out_expr) {
        ...
        find_first = true;
        dfs(cur);
        if (found) out_expr = first_expr;
        return found;
    }

    void solve_all_or_first_normal(const vector<Node>& input, bool findFirstOnly) {
        found = false;
        first_expr.clear();
        best_exprs.clear();
        best_plus.clear();
        ...
        if (find_first) {
            if (found && !immediate_print) add_answer(first_expr);
        }
    }
};
...
static bool parse_mode_cmd(const string& line, bool& toRandom) {
    if (line == "random") { toRandom = true; return true; }
    if (line == "solution") { toRandom = false; return true; }
    return false;
}
...
int main() {
    ...
    while (true) {
        if (!randomMode) {
            ...
            cout << "输入模拟次数、数字个数、最小值、最大值（输入 solution 返回解题模式）：";
        }
        cout << flush;
        ...
        if (!randomMode) {
            ...
            if (NO_NEGATIVE_INTERMEDIATE) {
                bool has_neg = false;
                for (auto& nd : input) {
                    if (nd.num.has_ll && nd.num.ll < 0) { has_neg = true; break; }
                }
                if (has_neg) {
                    cout << "??\n";
                    continue;
            ...
            if (!solver.found) {
                ...
                for (auto& kv : solver.best_exprs) print_infix(kv.second);
            }
            continue;
        }
        ...
        for (int t = 0; t < T; t++) {
            ...
            nums.reserve(N);
            for (int i = 0; i < N; i++) nums.push_back(dist(rng));
